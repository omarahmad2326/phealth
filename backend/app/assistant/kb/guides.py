...
## Where to find it
Open the site from Sites. Four cards at the top count sites: Passed (something has passed and nothing is failing), Failed (any equipment failed or red-tagged), Upcoming (an inspection visit scheduled and not started, or equipment falling due in the next 30 days) and In progress (an inspection visit started and not finished). Clicking a card shows only those sites; clicking it again shows them all. Each site is one row with its status - Passed all, Passed, Failed or Not inspected yet - and its overdue and due counts. Click the row and it drops open into a card per department showing how many of its equipment passed out of the total, and how many failed, plus Not in a department and Fleet when they hold anything. Clicking a department card opens that department; Open site goes into the site. Super Admins and Admins also see Delete site in the open row: type the site's name to confirm, and the site leaves Sites and every list, while its inspections, service jobs and other records are kept; the assistant cannot delete a site. One row is open at a time, and searching down to a single site opens it. The bar under the page title shows the site's name, Inspections, Service, Facility and Compliance. Open Facility and choose a category. The site's own page also shows a tile for each category with how many items it has, their total book value and how many need attention. The Back arrow left of the page title returns to the previous screen. The side bar's first buttons are Sites and the site you are in. Its menu holds only Sites and Inspection status, then People, Commerce and Workspace; a site's own screens are in the bar under the page title, whose menus also hold Inspection forms (under Inspections), the Asset Register, Assets & Value, Parts & Spares and Test Equipment (under Facility) and Contractors (under Compliance). People here, on the site's page, lists everyone assigned to the site, in any role, and nobody else.
